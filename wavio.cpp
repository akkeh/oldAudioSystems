...
sig read(const char* filename, int print) {
    int error = -1;
...
int write(const char* filename, sig const& in_sig, unsigned short bps, int print) {
    int error = -1;
    FILE* outfile=fopen(filename, "wb");
    if(!outfile) { printf("file %s not found", filename);}

    unsigned int ck3_size = in_sig.N * 2;
    ...
    double* f_data = (*(sig1d*)in_sig.data).re;
